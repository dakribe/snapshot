...
import { getPwhlGame } from '../../../lib/pwhl';

const pwhlGameQuery = query(getPwhlGame, 'pwhl-game');

export const route = {
  preload: ({ params }) => void pwhlGameQuery(params.id!),
} satisfies RouteDefinition;

function PwhlGameContent(props: { id: string }) {
  const game = createMemo(() => pwhlGameQuery(props.id));
  ...
  return (
    <>
      ...
      <section class="match-hero" aria-labelledby="pwhl-match-title">
        ...
        <div class="matchup">
          ...
          <div class="match-score" id="pwhl-match-title">
      ...
      <section class="data-panel pwhl-note"><h2>Game information</h2><p class="panel-empty">Live scoring and game events will appear here during the game.</p></section>
    </>
  );
}

export default function PwhlGamePage(props: RouteProps<'/pwhl/game/:id'>) {
  return (
    <main class="page-shell game-page">
      <Errored fallback={(error, reset) => <div class="error-state"><strong>Couldn’t load this game</strong><span>{String(error())}</span><button onClick={reset}>Try again</button><a href="/">Back to schedule</a></div>}>
        <Loading fallback={<div class="game-loading" aria-label="Loading game"><span /></div>}>
          <PwhlGameContent id={props.params.id} />
        </Loading>
      </Errored>
    </main>
  );
